...
class RepositoryExtractor:
    def __init__(self, g: Github, owner: str, name: str, path: str):
        self.repo = g.get_repo(f"{owner}/{name}")
        
        self.clone_path = os.path.join(path, "auto_extract", "repo", owner)
        self.repo_path = os.path.join(self.clone_path, name)
        
        self.save_path = os.path.join(path, "auto_extract", "save", owner, name)
        ...
        self.commit_ids = get_commit_hashes()[::-1]
